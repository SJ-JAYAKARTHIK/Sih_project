...
try {
  // Clean up any old test bookings for testDateStr
  db.data.bookings = db.data.bookings.filter(b => b.date !== testDateStr && !b.id.startsWith("TEST-"));
  db.save();
  ...
  const b1 = db.createBooking({ farmerId: farmer1Id, mandiId: mandiA, cropId: "crop-1", date: testDateStr, timeSlot: "09:00 - 09:30" });
  const b2 = db.createBooking({ farmerId: farmer2Id, mandiId: mandiA, cropId: "crop-3", date: testDateStr, timeSlot: "09:30 - 10:00" });
  const b3 = db.createBooking({ farmerId: farmer3Id, mandiId: mandiB, cropId: "crop-2", date: testDateStr, timeSlot: "09:00 - 09:30" });

  console.log("\n1. ACTIVE QUEUE ENTRY & EXCLUSION RULES");
  // Test 1: Unverified bookings must NOT be in active queue
  let qA = db.getMandiActiveQueue(mandiA, testDateStr);
  ...
  b1.arrivedAt = new Date().toISOString();
  ...
  b3.arrivedAt = new Date().toISOString();
  db.save();

  qA = db.getMandiActiveQueue(mandiA, testDateStr);
  ...
  const qB = db.getMandiActiveQueue(mandiB, testDateStr);
  ...
  db.startProcurement(mandiA, b1.id);
  qA = db.getMandiActiveQueue(mandiA, testDateStr);
  ...
  const f1Status = db.getFarmerQueueStatus(farmer1Id);
  const f2Status = db.getFarmerQueueStatus(farmer2Id);
  ...
  db.processProcurement(mandiA, { bookingId: b1.id, actualQty: 48.5, billedBy: "Officer Sharma" });
  qA = db.getMandiActiveQueue(mandiA, testDateStr);
  ...
  const bCancel = db.createBooking({ farmerId: farmer1Id, mandiId: mandiA, cropId: "crop-1", date: testDateStr, timeSlot: "11:00 - 11:30" });
  db.cancelBooking(farmer1Id, bCancel.id);
  qA = db.getMandiActiveQueue(mandiA, testDateStr);
  ...
  const b4 = db.createBooking({ farmerId: farmer1Id, mandiId: mandiA, cropId: "crop-1", date: testDateStr, timeSlot: "14:00 - 14:30" });
  const futureDate = "2029-12-30";
  db.rescheduleBooking(farmer1Id, b4.id, { newDate: futureDate, newTimeSlot: "10:00 - 10:30" });
  qA = db.getMandiActiveQueue(mandiA, testDateStr);
  ...
  const avgTime = db.calculateAverageProcessingTime(mandiA, testDateStr);
  assert(typeof avgTime === 'number' && avgTime > 0, `Estimated processing duration is valid number: ${avgTime} min`);

  // Clean up test bookings from store.json after test completion
