...
try {
  // Clean up any previous test bookings for test date
  ...
  // Verify Gate Arrival
  db.verifyArrival(mandi1Id, { tokenNumber: b1.tokenNumber });

  // Process procurement with Actual Quantity = 47.2 Quintals (4720 kg) instead of 50
  const procResult = db.processProcurement(mandi1Id, {
    bookingId: b1.id,
    actualQty: 47.2,
  ...
  assert(b1.actualQty === 47.2, "Booking record reflects actual quantity (47.2 Quintals)");

  // Verify consistency across Mandi history & Admin stats
  const mandiHistory = db.getMandiHistoryForDate(mandi1Id, testDate);
  const matchedHistBooking = mandiHistory.bookings.find(b => b.id === b1.id);
  assert(matchedHistBooking.actualQty === 47.2, "Mandi history uses actual quantity");
  assert(matchedHistBooking.pricePaid === expectedAmount, "Mandi history uses backend calculated final amount");

  // 2. DUPLICATE PROCUREMENT / BILLING REJECTION
  console.log("\n2. DUPLICATE PROCUREMENT / BILLING REJECTION");
  let duplicateProcError = false;
  try {
    db.processProcurement(mandi1Id, {
      bookingId: b1.id,
      actualQty: 47.2,
      billedBy: "Rajesh Officer"
    });
  ...
  const complaint1 = db.createComplaint({
    farmerId: farmer1Id,
    bookingId: b1.id,
  ...
  const farmer1Complaints = db.getFarmerComplaints(farmer1Id);
  const farmer2Complaints = db.getFarmerComplaints(farmer2Id);
  ...
  console.log("\n5. MANDI ISOLATION AUTHORIZATION FOR COMPLAINTS");
  const mandi1Complaints = db.getMandiComplaints(mandi1Id);
  const mandi2Complaints = db.getMandiComplaints(mandi2Id);
  ...
  const underReviewComp = db.updateComplaintStatus({
    complaintId: complaint1.id,
  ...
  const resolvedComp = db.updateComplaintStatus({
    complaintId: complaint1.id,
    updatedBy: "Mandi 1 Officer",
    role: "MANDI_OFFICER",
    mandiId: mandi1Id,
  ...
  console.log("\n7. ADMIN MULTI-FILTER COMPLAINT ACCESS");
  const adminComplaints = db.getAllComplaints({ mandiId: 'ALL', status: 'Resolved' });
