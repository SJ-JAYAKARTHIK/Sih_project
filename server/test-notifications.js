...
try {
  ...
  db.data.bookings = db.data.bookings.filter(b => b.date !== notifTestDate);
  ...
  const b1 = db.createBooking({ farmerId: farmer1Id, mandiId: mandiA, cropId: "crop-1", date: notifTestDate, timeSlot: "09:00 - 09:30" });
  
  let f1Notifs = db.getFarmerNotifications(farmer1Id);
  ...
  assert(db.getFarmerUnreadCount(farmer1Id) > 0, "Farmer 1 unread count is greater than 0");

  console.log("\n2. ARRIVAL VERIFICATION NOTIFICATION (STEP 2 & 3: ARRIVED & WAITING)");
  b1.date = new Date().toISOString().split('T')[0]; // Set to today for verifyArrival
  db.verifyArrival(mandiA, { tokenNumber: b1.tokenNumber });
  
  f1Notifs = db.getFarmerNotifications(farmer1Id);
  ...
  db.processProcurement(mandiA, { bookingId: b1.id, actualQty: 52.0, billedBy: "Officer Verma" });
  f1Notifs = db.getFarmerNotifications(farmer1Id);
  ...
  const b2 = db.createBooking({ farmerId: farmer2Id, mandiId: mandiA, cropId: "crop-3", date: notifTestDate, timeSlot: "11:00 - 11:30" });
  db.updateBookingStatus(mandiA, b2.id, { action: 'CANCEL' });
  let f2Notifs = db.getFarmerNotifications(farmer2Id);
  ...
  db.addNotification({ farmerId: farmer1Id, bookingId: b1.id, type: "PROCUREMENT_COMPLETED", title: "Duplicate Test", message: "Duplicate" });
  ...
  const unreadBefore = db.getFarmerUnreadCount(farmer1Id);
  const targetNotif = f1Notifs[0];
  db.markNotificationAsRead(farmer1Id, targetNotif.id);
  const unreadAfter = db.getFarmerUnreadCount(farmer1Id);
  ...
  try {
    ...
    db.markNotificationAsRead(farmer2Id, targetNotif.id);
  } catch (e) {
